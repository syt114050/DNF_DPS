/*
文件名:光环回收.nut
路径:OfficialProject/光环回收/光环回收.nut
创建日期:2026-05-18
文件用途:使用光环回收卷，将时装栏(INVENTORY_TYPE_AVARTAR=2)指定范围内的光环按配置规则回收为奖励道具。
         支持指定ID匹配和品级+等级匹配两种模式，命中重叠时优先指定ID。
*/

// ========== 辅助函数 ==========

function _Dps_AuraRecycle_GetText_(Config, Key, DefaultText) {
    if (Config.rawin("提示文本") && Config["提示文本"].rawin(Key)) return Config["提示文本"][Key];
    return DefaultText;
}

function _Dps_AuraRecycle_IsExcluded_(ExcludeList, EquipId) {
...
function _Dps_AuraRecycle_DeleteDb_(Cid, SlotList) {
...
function _Dps_AuraRecycle_Func_(SUser, ItemId) {
    local Config = GlobalConfig.Get("光环回收配置.json");
    ...
    for (local i = RangeStart; i <= RangeEnd; i++) {
        ...
        // 排除列表检查
        if (_Dps_AuraRecycle_IsExcluded_(ExcludeList, EquipId)) continue;
        ...
        foreach (rule in Config["指定装备回收"]) {
            if (rule[0] == EquipId) {
                ...
                if (RewardId == 0) {
                    ...
                    _AuraRecycle_Helper.sendRewardMessageForCera(SUser, EquipName, ItemObj, count, EquipId);
                } else {
                    ...
                    _AuraRecycle_Helper.api_CUser_Add_Item_list(SUser, totalRewards);
                    _AuraRecycle_Helper.sendRewardMessageForItem(SUser, EquipName, ItemObj, RewardName, count, equType, EquipId, RewardId);
                }
    ...
    // --- 数据库清理 ---
    _Dps_AuraRecycle_DeleteDb_(Cid, deletedSlots);

    // --- 结果通知 ---
    if (foundValid) {
        SUser.SendItemSpace(1);
        if (index > 0) {
            SUser.SendNotiPacketMessage("恭喜: " + index + " 件光环回收成功。", Config["信息播报发送位置"]);
            if (allRewards.len() > 0) {
                _AuraRecycle_Helper.SendItemWindowNotification(SUser, allRewards);
            }
        }
        ...
        _AuraRecycle_Helper.RecycleError(SUser, Config["回收失败信息"], ItemId);
    }
}

// ========== 辅助类 ==========

class _AuraRecycle_Helper {
    ...
    function sendRewardMessageForCera(SUser, EquipName, ItemObj, count, EquipId) {
        ...
        AdMsgObj.PutEquipment("[" + EquipName + "]", ItemObj, _AuraRecycle_Helper.RarityColor(EquipId));
    ...
    function sendRewardMessageForItem(SUser, EquipName, ItemObj, RewardName, count, equType, EquipId, RewardId) {
        ...
        AdMsgObj.PutString(Config["成功回收"]["奖励提示"]);
        if (equType > 0) {
            AdMsgObj.PutColorString("[" + RewardName + "]", _AuraRecycle_Helper.RarityColor(RewardId));
        } else {
            AdMsgObj.PutColorString("[" + count + "]", [255, 20, 0]);
    ...
    function RarityColor(itemId) {
        ...
        return _AuraRecycle_Helper.rarityColorMap[(rarity).tostring()];
    }
...
function _Dps_AuraRecycle_Logic_() {
    local Config = GlobalConfig.Get("光环回收配置.json");
    foreach (BoxId in Config["光环回收卷ID"]) {
        Cb_Use_Item_Sp_Func[BoxId] <- _Dps_AuraRecycle_Func_;
    }
}

function _Dps_AuraRecycle_Main_() {
    _Dps_AuraRecycle_Logic_();
}

function _Dps_AuraRecycle_Main_Reload_(OldConfig) {
    foreach (BoxId in OldConfig["光环回收卷ID"]) {
