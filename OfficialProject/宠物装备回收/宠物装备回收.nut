/*
文件名:宠物装备回收.nut
路径:OfficialProject/宠物装备回收/宠物装备回收.nut
创建日期:2026-05-17
文件用途:使用宠物装备回收卷，将宠物栏指定范围内的宠物装备按配置规则回收为奖励道具
*/

// ========== 辅助函数 ==========

function _Dps_PetEquipRecycle_GetText_(Config, Key, DefaultText) {
    if (Config.rawin("提示文本") && Config["提示文本"].rawin(Key)) return Config["提示文本"][Key];
    return DefaultText;
}

function _Dps_PetEquipRecycle_IsExcluded_(ExcludeList, EquipId) {
...
function _Dps_PetEquipRecycle_DeleteDb_(Cid, SlotList) {
    if (SlotList.len() == 0) return;
    local SlotStr = "";
    for (local i = 0; i < SlotList.len(); i++) {
        if (i > 0) SlotStr += ",";
        SlotStr += SlotList[i].tostring();
    }
    local Sql = "delete from taiwan_cain_2nd.creature_items where charac_no=" + Cid + " and slot in (" + SlotStr + ");";
...
function _Dps_PetEquipRecycle_Func_(SUser, ItemId) {
    local Config = GlobalConfig.Get("宠物装备回收配置.json");
    ...
    for (local i = RangeStart; i <= RangeEnd; i++) {
        ...
        // 排除列表检查
        if (_Dps_PetEquipRecycle_IsExcluded_(ExcludeList, EquipId)) continue;
        ...
        foreach (rule in Config["指定装备回收"]) {
            if (rule[0] == EquipId) {
                ...
                if (RewardId == 0) {
                    ...
                    _PetEquipRecycle_Helper.sendRewardMessageForCera(SUser, EquipName, ItemObj, count, EquipId);
                } else {
                    ...
                    _PetEquipRecycle_Helper.api_CUser_Add_Item_list(SUser, totalRewards);
                    _PetEquipRecycle_Helper.sendRewardMessageForItem(SUser, EquipName, ItemObj, RewardName, count, equType, EquipId, RewardId);
                }
    ...
    // --- 数据库清理 ---
    _Dps_PetEquipRecycle_DeleteDb_(Cid, deletedSlots);
    ...
    if (foundValid) {
        ...
        if (index > 0) {
            ...
            if (allRewards.len() > 0) {
                _PetEquipRecycle_Helper.SendItemWindowNotification(SUser, allRewards);
            }
        }
        ...
        _PetEquipRecycle_Helper.RecycleError(SUser, Config["回收失败信息"], ItemId);
    }
}

// ========== 辅助类 ==========

class _PetEquipRecycle_Helper {
    ...
    function sendRewardMessageForCera(SUser, EquipName, ItemObj, count, EquipId) {
        ...
        AdMsgObj.PutEquipment("[" + EquipName + "]", ItemObj, _PetEquipRecycle_Helper.RarityColor(EquipId));
    ...
    function sendRewardMessageForItem(SUser, EquipName, ItemObj, RewardName, count, equType, EquipId, RewardId) {
        ...
        AdMsgObj.PutString(Config["成功回收"]["奖励提示"]);
        if (equType > 0) {
            AdMsgObj.PutColorString("[" + RewardName + "]", _PetEquipRecycle_Helper.RarityColor(RewardId));
        } else {
            AdMsgObj.PutColorString("[" + count + "]", [255, 20, 0]);
    ...
    function RarityColor(itemId) {
        ...
        return _PetEquipRecycle_Helper.rarityColorMap[(rarity).tostring()];
    }
...
function _Dps_PetEquipRecycle_Logic_() {
    local Config = GlobalConfig.Get("宠物装备回收配置.json");
    foreach (BoxId in Config["宠物装备回收卷ID"]) {
        Cb_Use_Item_Sp_Func[BoxId] <- _Dps_PetEquipRecycle_Func_;
    }
}

function _Dps_PetEquipRecycle_Main_() {
    _Dps_PetEquipRecycle_Logic_();
}

function _Dps_PetEquipRecycle_Main_Reload_(OldConfig) {
    foreach (BoxId in OldConfig["宠物装备回收卷ID"]) {
